"""
grid_search_for_paper.py

- Prints grid values including specific numbers and total configurations
- Plots parameter sensitivity for all algorithms (x-axis labeled with actual parameter names, with error bars)
- All figures in English
- Adds statistical significance tests for wfr_smc and wfr_bdl
"""
...
# 1. Device and global configuration
...
# 3. Parameter estimation and core computations
...
# 4. One-step updates for each algorithm
...
def perform_grid_search():
    ...
    #  Print full grid value table and number of configurations per algorithm
    ...
    for mode, grid in param_grids.items():
        ...
        for k, v in grid.items():
            if isinstance(v[0], (int, float)):
                ...
                # For list of tuples, print structure
                print(f"  Param {k}: {len(v)} values")
...
# 7. Plotting functions (sensitivity plots for all algorithms, convergence curves, significance tests)

def plot_validation_curves(results_list, all_combo_records, base_params, param_grids):
    n_repeats = 5          # Number of repetitions per parameter combination
    K_opt = 300            # Steps for convergence curves
    K_sens = 150           # Steps for sensitivity plots (same as grid search)

    
   # Plot parameter sensitivity for all algorithms (with error bars)
  
    print("Plotting parameter sensitivity maps for all algorithms (mean ± std, n_repeats = {})...".format(n_repeats))

    # Parameter axis label mapping (English), using Matplotlib mathtext format
    axis_labels = {
    ...
    for mode in param_grids.keys():
        ...
        # Determine the key parameter to plot (for multi-parameter algorithms, fix others to optimal)
        if mode in ['underdamped_pro', 'vgd', 'wfr_bdl']:
            # Find the best record for this mode
            ...
            # Select records where all other parameters equal the best values (only key parameter varies)
            ...
            if filtered_records:
                ...
                selected_records = records   # Fallback to all records
        else:
        ...
        # For each selected parameter combination, run n_repeats times to compute mean and std
        ...
        for p, _ in selected_records:
            ...
            for rep in range(n_repeats):
                torch.manual_seed(1000 + rep * 10)   # Different seeds for each repetition
                theta_init = torch.randn(params['p'], X_train.shape[1], device=device)
            ...
            if scores:   # At least one valid run
        ...
        # Sort by key parameter value for plotting
        ...
        # Mark the optimal point (minimum mean)
    ...
    # Convergence curves and significance test 
